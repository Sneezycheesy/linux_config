...
myScratchpads = [NS "social" spawnChromium findChrom floatChrom,
    ...
    where
      ...
      manageTsm = customFloating $ W.RationalRect l t w h
              where
               h = 0.8
               w = 0.8
               t = 0.9 - h
...
myKeys conf@XConfig {XMonad.modMask = modm} =
  M.fromList $
    ...
    [ ((modm .|. shiftMask, xK_Return), spawn $ XMonad.terminal conf),
      ...
      ((modm .|. alt, xK_3), spawn $ "sh " ++ scriptDir ++ "volume.sh sink vr"),
      ((modm .|. alt, xK_4), spawn $ "sh " ++ scriptDir ++ "volume.sh sink bluetooth"),
      ((modm .|. control, xK_1), spawn $ "sh " ++ scriptDir ++ "volume.sh source headset"), -- Input headset (USB device)
      ((modm .|. control, xK_2), spawn $ "sh " ++ scriptDir ++ "volume.sh source vr"), -- Input headset (USB device)
      ((modm .|. control, xK_3), spawn $ "sh " ++ scriptDir ++ "volume.sh source record"),
      ...
      ((modm, xK_o), namedScratchpadAction myScratchpads "corectrl"),
      ((modm .|. control, xK_t), namedScratchpadAction myScratchpads "tsm"),
